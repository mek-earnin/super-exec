...
2. **Discuss (spec).** Interview the user relentlessly to reach shared understanding of the WHAT. Research the codebase via subagents instead of asking what can be discovered. Detect new-vs-update: glob `docs/specs[/<app>]`, match by ticket key in the spec header + feature slug after the numeric prefix, confirm with the user; an update interviews only the delta. Sharpen terminology, but do not write the final spec file until the ticket value is resolved for its header. **One session can yield one or more specs — one per cohesive feature.** When the interview covers several distinct features (e.g. a new app described by its feature set), propose a split via a gate before writing (never split silently; the user can always keep it as a single spec). Cohesion — a shared design intention — sets the boundary, not PR size: tightly-coupled capabilities (login/logout/session) stay one spec; independent capabilities (auth/profile/feed) become separate specs. The split's specs are numbered in dependency order, and a spec may record an optional `Depends on` sibling.
...
| Plan(s) | `.super-exec/NNNN-<feature>/<YYYY-MM-DD>-<plan-name>/plan.md` (monorepo: `.super-exec/<app>/NNNN-<feature>/…`, mirroring the per-app spec path) with plan frontmatter (`title`, `feature`, `branch`, `status`) | **no (gitignored)** | local |
| Handoff / scratch | `.super-exec/NNNN-<feature>/<…>/{handoff.md,scratch/}` | no | local |

One long-lived spec per feature; many dated plans per feature (≈ one plan ≈ one execute session ≈ one PR). A single discuss session may produce several specs (one per cohesive feature); they are batch-approved and committed together on the first feature's branch, then planned one at a time via `/se-plan` next-unplanned discovery. Because all specs ride the first feature's branch, a later feature must branch off the base only after that first PR merges (so the shared specs are present on the base) — otherwise it branches off the prior feature's branch; se-plan enforces that the selected spec is present on the chosen base before creating the branch. **The spec never references the plan** (plans are local; teammates have only specs). `NNNN` is the next available four-digit number in that specs directory (find the highest existing prefix and increment), while `<feature>` is the work's feature name — the same slug as the branch (a ≤6-word summary of the change), lowercase-kebab; it is **never** the repo/project/package name, and the tool halts to confirm with the user if a candidate slug collides with the project name. The plan directory reuses the spec's **full name** including the number prefix — `.super-exec/NNNN-<feature>/` — so the local plan folder matches its governing spec at a glance. For build sessions, the branch source of truth is plan frontmatter `branch`; `.super-exec/active` is only the local resume pointer. The tool keeps `.super-exec/` out of git via the repo-local, **uncommitted** `.git/info/exclude`, so the local-only artifact gets a local-only ignore with no edit to the team-shared `.gitignore`. The `se-local-ignore` skill owns this and is applied by each driver skill at session activation (the guaranteed layer); the SessionStart hook does the same as a best-effort backup. (See ADR 0002.)
...
`Depends on` is optional (`none` when there is no dependency): a split feature names the sibling spec it needs, and the numbering keeps a dependency ahead of the feature that needs it.
...
- **Next-unplanned discovery** — se-plan's no-argument behavior that selects the lowest-numbered spec without a plan directory, so a split's remaining specs are planned one at a time.
