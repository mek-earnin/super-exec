...
Artifact placement is **config-driven** (`se-config`: `commitSpec`, `commitPlan`; each `true | false | "ask"`) and uses **symmetric roots** — `<root>` ∈ {`docs/specs/` (committed, team), `.super-exec/specs/` (local, gitignored)} — with identical per-feature sub-structure. Identity is **slug-only** (no running number); every locator scans BOTH roots.
