...
2. **Discuss (spec).** Interview the user relentlessly to reach shared understanding of the WHAT. Research the codebase via subagents instead of asking what can be discovered. Detect new-vs-update: glob BOTH spec roots (`docs/specs[/<app>]` and `.super-exec/specs[/<app>]`), match by ticket key in the spec header + feature slug (slug-only folder, no numeric prefix), confirm with the user; an update interviews only the delta. Sharpen terminology, but do not write the final spec file until the ticket value is resolved for its header. **One session can yield one or more specs — one per cohesive feature.** When the interview covers several distinct features (e.g. a new app described by its feature set), propose a split via a gate before writing (never split silently; the user can always keep it as a single spec). Cohesion — a shared design intention — sets the boundary, not PR size: tightly-coupled capabilities (login/logout/session) stay one spec; independent capabilities (auth/profile/feed) become separate specs. Each split spec gets its own slug folder (no running number), and dependency order is conveyed by an optional `Depends on` sibling header rather than by numbering.
...
4. **Write review artifacts.** For a new spec, resolve its root (`docs/specs/` or `.super-exec/specs/`) through `commitSpec`; for an existing-spec update, preserve the exact discovered path/root without consulting config or re-prompting about placement. Only an explicit user request may move an existing spec between roots. Update `CONTEXT.md` when new terms need review, and offer/write an ADR only when the decision is hard to reverse + surprising + a real trade-off. The written files are the review artifacts; do not commit them before approval.
5. **Spec-approval gate, then branch + commit.** Present the finalized spec file and ask the user to review — "Everything look good? Proceed to planning?". A change request loops back to editing the written file and re-presenting; repeat until the user explicitly approves. On approval, create/switch to the already-confirmed branch, then **commit the approved artifacts** (the spec, `CONTEXT.md`, ADRs as needed) via `se-commit` — *unless* the user asks not to commit, or the spec's resolved on-disk root is local (`.super-exec/specs/`, which is never committed). The approval is authorization to create the confirmed branch and commit; do not ask again unless branch creation fails, facts changed, or checkout conflicts with the approved uncommitted files. In that conflict case, stop and ask; never stash, discard, or rewrite approved review artifacts silently. **For a multi-feature split:** batch-approve all specs together, ask which to plan first (default the first by `Depends on` order), create that first feature's branch, and commit every spec whose resolved on-disk root is `docs/specs/` in one commit on it — so the first PR carries the first feature's implementation plus all the committed specs. Only the first-chosen feature auto-chains into planning; the rest are picked up later, one at a time, via `/se-plan` next-unplanned discovery.
6. **Plan (architecture + verification).** Once the spec is approved, **se-discuss auto-chains into the Plan phase in the same session — the user never has to invoke `/se-plan` manually.** Read the spec. Design the architecture, researching via subagents. Discover the full repo skill catalog and bind matching skills to tasks (see Repo skill catalog & task binding). **Resolve every risk and open question with the user up front** (grill like discuss, but for HOW) — the plan carries no open items into execution. Design verification (see below) and reach agreement. If the feature involves UI work, incorporate impeccable automatically when it is installed — no user prompt (see Optional: UI design). Write the plan with plan frontmatter (`title`, `feature`, confirmed `branch`, `status: pending`) before `## Execution Checklist` as the first section, then immediately record `active_plan: <root>/[<app>/]<feature>/plans/<YYYY-MM-DD>-<plan-name>/plan-<plan-name>.md` in `.super-exec/active`. **Human-review gate:** the user reviews architecture + data flow. At approval, `se-plan` may add/update `approved:` metadata, but approval is not the first time `active_plan` is recorded; the plan status stays `pending` until execution starts.
...
`/se-plan` is also a manual re-entry point: re-plan or update an existing spec's plan without re-interviewing the spec. With **no argument** it picks up the **next unplanned spec** — a spec (in either root) that has no plan folder yet, listed for the user to choose (with `Depends on` giving ordering) rather than auto-picked by number — and it **creates that feature's branch** when the spec has none (features 2..N of a split, or a re-entry from a base branch). This is how a split's remaining specs are planned and built one PR at a time. If the selected spec's `Depends on` sibling is not built yet, se-plan warns the user (non-blocking) before planning.

### Session 2 — build

**`/se-exec`** drives the whole session:

1. Locate the relevant plan. `/se-exec` activation reads any existing `.super-exec/active` first and refreshes it with `phase: exec` while preserving any existing `active_plan` (and convenience `branch`) until plan resolution replaces it. If `/se-exec` was manually invoked with an explicit plan path (the plan file `plan-<plan-name>.md`, or a directory containing it), use that plan without asking for confirmation; the path is the confirmation. If no path was specified, check `.super-exec/active` for `active_plan`, resolve it, and ask the user to confirm that plan. If there is no active-plan marker, match branch/ticket → feature → most recent plan dir, then confirm the inferred match. If no plan exists, or the marker points at a missing plan, tell the user and stop. Once selected, refresh `.super-exec/active` with `phase: exec` and the selected `active_plan`. Read plan frontmatter, checklist, and spec for acceptance; if frontmatter has `branch`, the current git branch must match in normal mode, while `--worktree` validates the main checkout before creating the worktree and then lets worktree setup own branch checkout. A missing checklist is never treated as completed: completion requires frontmatter `status: completed`, or the first `## Execution Checklist` exists, has at least one item, and all items are checked. If no checklist exists and status is not completed, resume from `## Tasks` / handoff evidence or ask the user to confirm the resume point. At execution start, update plan `status` from `pending` to `in_progress`. Seed or sync the harness-native task/todo list from the plan's `## Execution Checklist` when available, using the CC `TodoWrite` primitive or its translated equivalent. The native task/todo tool is operational session state; plan frontmatter + checklist are durable local resume state.
...
3. **Outer loop — review / fix**, wrapping an **inner loop — execute / verify**:
   ...
   - **Code review:** an arch-gate pass first — does the implementation match the agreed architecture? Deviation → fix (re-enters the inner loop) → re-gate. Once the shape is sound, a deep-review pass: data-flow bugs → reuse-existing-utility check → pattern/consistency, plus an impeccable UI-critique lens on UI work whenever impeccable is installed. Findings are severity-tagged (Critical / Important / Minor). Critical/Important block and re-enter the inner loop; Minor is reported, non-blocking. A fresh reviewer re-reviews after fixes. Loop until clean.
...
`/se-pr-triage` is the fourth entrypoint and owns post-PR review/CI triage only. Track A uses two independent human gates: Gate 1 approves or steers comment decisions and Fix scope, contains no draft reply message, and treats its rationale as internal analysis only; after approved fixes are verified, reviewed, committed, and pushed, reply composition begins and Gate 2 presents the exact final target/body for every reply and waits for explicit approval before posting. Gate 1 never authorizes replies, and neither gate auto-approves under `/loop`. Track B resolves CI failures autonomously and never replies. The skill writes `.super-exec/active` with `phase: pr-triage` on activation, processes one triage round, and clears its session markers at every exit path (no PR/access failure/nothing actionable/end-of-round). It is not auto-chained from `se-pr`; the human invokes it explicitly or wraps it in `/loop`.

**Resume:** the controller monitors its own context. At a threshold it auto-writes a handoff file (`handoff-<plan-name>.md`, via super-exec's own `se-handoff` skill, into the plan folder) and pauses; the user runs `/clear` and re-runs `/se-exec`, which detects the active plan marker + handoff and resumes from the next unstarted task. Each outer-loop-clean task is also marked in the plan's first `## Execution Checklist` section; if every checklist item is complete, `/se-exec` tells the user the plan is already completed instead of restarting it.
...
`se-discuss`/`se-plan` first classifies whether the feature involves UI work (a frontend app, or the change touches components/styles). **If there is no UI work, impeccable is never used.** If there is UI work, super-exec **uses `impeccable` automatically whenever it is installed — it never asks the user whether to use it.** The UI classification is persisted in the plan and honored across three touchpoints; availability is re-detected at each touchpoint, so no phase (including `se-exec`) ever prompts the user about impeccable.
...
**Availability (detect-and-use, never ask):** impeccable may not be installed on every machine. super-exec detects it at each touchpoint; on a UI feature it is used automatically when present, and when absent it skips the UI passes gracefully, reports that it skipped and why, and suggests installing it — it never blocks the workflow and it never asks the user whether to use it. All three touchpoints invoke impeccable via a subagent to keep the controller lean; interactive `craft` iteration is surfaced to the user when needed.
...
6. **No commit while a human-review gate is open; no PR reply without exact post-fix approval.** Routine commits are expected (per-task auto-commit, the spec commit on approval, an Auto-PR-mode `docs:` amend); what is forbidden is committing while a review gate (`.super-exec/gate-open`) is open, and replying to a PR comment before Gate 2 explicitly approves its exact post-fix target and body. Gate-1 decision/Fix approval is never reply approval.
...
Enforcement layers: strong directive language + red-flag tables + ordered checklists in skill bodies; a mandatory TodoWrite checklist per phase; and selective pre-execution hooks active only during a super-exec session (marker `.super-exec/active`) — **nudge** (non-blocking) on heavy build/test run in the controller (N1), on raw git/gh commands (N2), and on writing fenced code into a spec file (`spec-<feature>.md`, under `docs/specs/` or `.super-exec/specs/`) (N3 — "spec is WHAT-only; move code to the plan"); and one **block** (hard) — B1, on commit while a human-review gate is open. The two PR-reply approval gates are enforced by explicit skill instructions, ordered steps, and red-flag corrections rather than a hook-level block. Cursor enforces B1 natively where its hook API supports `deny`; N1/N2/N3 still degrade to prose where Cursor lacks the needed allow-path context, controller signal, or generic file-write event.

## Artifacts & locations

New-artifact placement is **config-driven** (`se-config`: `commitSpec` — `true | false | "ask"` — and `commitPlan` — `"inheritSpec" | false | "ask"`); existing-spec updates preserve their discovered root. Both use **symmetric roots** — `<root>` ∈ {`docs/specs/` (committed, team), `.super-exec/specs/` (local, gitignored)} — with identical per-feature sub-structure. Identity is **slug-only** (no running number); every locator scans BOTH roots.

| Artifact | Location | Committed? | Visibility |
|---|---|---|---|
| Branch context | `.super-exec/branch-context.json` or in-memory worker result | no | local |
| Spec | `<root>/[<app>/]<feature>/spec-<feature>.md` (`<root>` from `commitSpec` when new; existing root when updated) | by resolved on-disk root | team when committed |
| ADR | global `docs/adr/<slug>.md`; feature-specific `<root>/[<app>/]<feature>/adr/<slug>.md` (slug-only, sparingly) | per scope/root | team when committed |
| Glossary | `CONTEXT.md` / `CONTEXT-MAP.md` | yes, lazy | team |
| Active marker | `.super-exec/active` (`phase`, `active_plan` — the plan file path — immediately after the plan file is written; optional `approved`; optional convenience `branch`) | no (gitignored) | local |
| Plan(s) | `<root>/[<app>/]<feature>/plans/<YYYY-MM-DD>-<plan-name>/plan-<plan-name>.md` (`<root>` per `commitPlan`) with plan frontmatter (`title`, `feature`, `branch`, `status`) | per `commitPlan` | team when committed |
| Handoff / scratch | `<root>/[<app>/]<feature>/plans/<…>/{handoff-<plan-name>.md,scratch/}` | follows the plan | local unless committed |

One long-lived spec per feature; many dated plans per feature (≈ one plan ≈ one execute session ≈ one PR). A single discuss session may produce several specs (one per cohesive feature); specs whose resolved on-disk root is `docs/specs/` are batch-approved and committed together on the first feature's branch, then planned one at a time via `/se-plan` next-unplanned discovery. Because committed specs ride the first feature's branch, a later feature must branch off the base only after that first PR merges (so the shared specs are present on the base) — otherwise it branches off the prior feature's branch; se-plan enforces that the selected spec is present on the chosen base before creating the branch. **The spec never references the plan** (a plan may be local, so teammates may have only the spec). Identity is **slug-only** — there is no running number: `<feature>` is the work's feature name (the same slug as the branch, a ≤6-word summary, lowercase-kebab, named per `/se-slug-naming`); it is **never** the repo/project/package name, and the tool halts to confirm with the user if a candidate slug collides with the project name. The per-feature folder holds the spec, any feature-scoped ADRs, and a `plans/` subtree — `<root>/[<app>/]<feature>/plans/<YYYY-MM-DD>-<plan-name>/` — so plans sit beside their governing spec: `commitPlan` is derived from the spec, not independent — it resolves against the governing spec's on-disk root (`"inheritSpec"` follows that root, `false` forces local, `"ask"` prompts only when the spec is committed), and a plan is never committed while its spec is local. For build sessions, the branch source of truth is plan frontmatter `branch`; `.super-exec/active` is only the local resume pointer. When an artifact resolves to the local root, the tool keeps `.super-exec/` out of git via the repo-local, **uncommitted** `.git/info/exclude`, so the local-only artifact gets a local-only ignore with no edit to the team-shared `.gitignore`. The `se-local-ignore` skill owns this and is applied by each driver skill at session activation (the guaranteed layer); the SessionStart hook does the same as a best-effort backup. (See the `config-driven-artifact-placement` ADR, which supersedes the `local-only-plans-specs-are-the-shared-contract` ADR.)
...
`Depends on` is optional (`none` when there is no dependency): a split feature names the sibling spec it needs (slug-only, no number), and `Depends on:` — not a running number — conveys which spec must be built first.

The spec is **WHAT-only**: no code blocks, no file layout, no scaffolding — those belong to the plan. A `PreToolUse` nudge (N3) reminds the agent when fenced code is written into a spec file (`spec-<feature>.md`, under either spec root) during a session. (This dogfood spec is itself *about* a workflow, so it legitimately quotes templates and trees in fences — the nudge is a reminder, not a hard block, precisely so spec-about-a-spec cases are not walled off.)
...
| handoff | bundled `se-handoff` (super-exec's own; writes `handoff-<plan-name>.md` in the plan folder, following the plan's root) |
...
Used only when the target repo has no skill/pattern for the action (per ADR `repo-conventions-win-tool-defaults-fallback`). super-exec ships these so it works in a bare repo.
...
**Commit**: owned entirely by the `se-commit` skill — the pre-commit review floor, staging isolation, plus the message (the repo commit skill, e.g. `git-commit-message`, else a conventional fallback). The fallback conventional types include `feat`, `fix`, `refactor`, `style`, `test`, `docs`, `ci`, and `chore`; performance-only changes are `refactor` when they do not change behavior. **Only reviewed changes may be committed, and every commit — inside super-exec or not — goes through `se-commit`:** it is the enforcement gate, not the reviewer. It requires that the staged files were reviewed first and refuses to commit when they were not; the committer, which holds the change's intent, does the actual reviewing. Commits covered by an `se-review` pass — se-exec's per-task build loop wraps a blocking review around each commit, and se-pr-triage's fix flow reviews before committing — need no separate re-review, and a human-approved spec is itself review evidence. The spec does not duplicate the full commit rules; see the `se-commit` skill for the authoritative definition.
...
├── docs/specs/core-workflow/spec-core-workflow.md  # this spec (dogfood; slug-only v1 layout)
...
This decision is recorded in ADR `superpowers-coinstall-skill-precedence`.

## Portability

Authored and dogfooded on Claude Code. Skills are written in **Claude Code language** — CC tool names (`Read`, `Edit`, `Write`, `Task`, `AskUserQuestion`, `TodoWrite`), model aliases (`opus` / `sonnet` / `haiku`, never pinned version strings), and hook event names. Cross-harness support is a **single one-way translation table** at `using-super-exec/references/cursor-tools.md`; individual skills do not branch by harness or spell alternate tool APIs inline. The table maps CC primitives — tools, model aliases (strong → best high-effort reasoning model; mid → latest cursor-grok-* high; cheap → latest composer non-fast), hook events, env vars — to each harness equivalent. Model tiers resolve through the internal `se-subagent` skill. Because Cursor runs had skipped the soft-referenced table and inherited the session model for every subagent, the SessionStart hook injects `cursor-tools.md` on Cursor. The table stays tier-named and slug-free; the actual slug is picked from the dispatch tool's allowlist at dispatch time. A `.cursor-plugin` config ships with the plugin. CC-only *nudge* hooks (N1/N2/N3) degrade to prose on Cursor; the B1 commit-gate hard block runs natively on both harnesses through shell pre-execution. PR-reply approval remains skill-enforced on both harnesses.

## Domain terms

- **Discuss** — the spec phase: interview to shared understanding of the WHAT.
- **Plan** — the architecture + verification design phase, and its plan file (`plan-<plan-name>.md`, committed or local per `commitPlan`). One feature has many plans over time.
- **Feature** — a long-lived unit of work with one spec; `commitSpec` chooses a new spec's root, while updates preserve its resolved on-disk root.
- **Feature-split gate** — the discuss-phase gate that proposes splitting a multi-feature session into one spec per cohesive feature; it never splits silently and the user can keep a single spec.
- **Cohesion boundary** — the rule that a spec's scope is a shared design intention (tightly-coupled capabilities together), not PR size; it decides how many specs a session produces.
- **Next-unplanned discovery** — se-plan's no-argument behavior that lists specs (across both roots) without a plan folder and asks which to plan (using `Depends on` for ordering), so a split's remaining specs are planned one at a time.
...
See `docs/adr/` for: repo-skill delegation precedence (`repo-conventions-win-tool-defaults-fallback`); local-only plans / spec-plan visibility boundary (`local-only-plans-specs-are-the-shared-contract`, superseded by `config-driven-artifact-placement`); skills-only entrypoints, no thin commands (`skills-only-no-thin-commands`); superpowers co-install skill-precedence (`superpowers-coinstall-skill-precedence`); and config-driven artifact placement — symmetric roots, slug-only identity, scoped ADRs (`config-driven-artifact-placement`).

## Related specs

- **`se-pr-triage`** — implemented as the fourth entrypoint and specced in [`spec-pr-triage-watch-bot.md`](../pr-triage-watch-bot/spec-pr-triage-watch-bot.md). One loop-safe triage round per invocation over two tracks: review comments (CodeRabbit + human) are triaged Fix/Decline/Answer/Defer, Gate 1 approves decisions/Fix scope, approved Fixes clear verify/review and are committed/pushed, then Gate 2 approves each exact final reply target/body before posting; CI failures (Playwright, Cycode, lint/type/unit, SonarCloud, PR-title) are investigated and resolved autonomously (flaky → rerun once; real → fix via the build verify/review loops), never replied to. Sequencing is strict: Gate 1 → fix/verify/review/commit/push → exact final reply preview → Gate 2 → post. The realism rule still rejects needless complexity for impossible cases.
