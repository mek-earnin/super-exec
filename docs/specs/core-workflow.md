...
2. **Discuss (spec).** Interview the user relentlessly to reach shared understanding of the WHAT. Research the codebase via subagents instead of asking what can be discovered. Detect new-vs-update: glob `docs/specs[/<app>]`, match by ticket key + name, confirm with the user; an update interviews only the delta. Sharpen terminology, but do not write the final spec file until the ticket value is resolved for its header.
...
| Spec | `docs/specs/<feature>.md` (monorepo app-specific: `docs/specs/<app>/<feature>.md`) | yes, after approval | team |
