...
2. **Discuss (spec).** Interview the user relentlessly to reach shared understanding of the WHAT. Research the codebase via subagents instead of asking what can be discovered. Detect new-vs-update: glob `docs/specs[/<app>]`, match by ticket key + name, confirm with the user; an update interviews only the delta. Sharpen terminology, prepare the spec draft, and offer an ADR only when the decision is hard to reverse + surprising + a real trade-off. Keep draft artifacts in memory or `.super-exec/` until the branch is ready; do not persist changes under `docs/specs/`, `docs/adr/`, or `CONTEXT.md` on the wrong branch.
3. **Ticket + branch confirmation.** After the last WHAT question and before presenting the final approval prompt, resolve traceability. If no ticket was provided or inferred, ask once: "Do you have a Jira ticket for this task?" If the user provides a ticket and Atlassian MCP is available, fetch the ticket data and derive the branch name from the repo convention plus the ticket title/final feature slug. If there is no ticket, use `NO_TICKET` in the spec and derive a no-ticket branch from the final feature slug. Present the ticket value and proposed branch name once for confirmation or edit.
4. **Spec-approval gate, then branch + commit.** Present the finalized spec and ask the user to review — "Everything look good? Proceed to planning?". A change request loops back to fixing the draft and re-presenting; repeat until the user explicitly approves. On approval, create/switch to the already-confirmed branch, then write/update the committed artifacts (`docs/specs/`, `CONTEXT.md`, ADRs as needed), and **commit the spec** via `se-commit` — *unless* the user asks not to commit, or `docs/specs/` is gitignored. The approval is authorization to create the confirmed branch and commit; do not ask again unless branch creation fails or facts changed.
5. **Plan (architecture + verification).** Once the spec is approved, **se-discuss auto-chains into the Plan phase in the same session — the user never has to invoke `/se-plan` manually.** Read the spec. Design the architecture, researching via subagents. Discover the full repo skill catalog and bind matching skills to tasks (see Repo skill catalog & task binding). **Resolve every risk and open question with the user up front** (grill like discuss, but for HOW) — the plan carries no open items into execution. Design verification (see below) and reach agreement. If the feature involves UI work, ask about incorporating impeccable (default yes — see Optional: UI design). Write the plan. **Human-review gate:** the user reviews architecture + data flow.
6. End: instruct the user to start a fresh session and run `/se-exec`. The discuss→plan transition auto-chains; the **plan→execute transition is a hard fresh-session boundary** and is never auto-chained.
...
│       ├── se-discuss/                 # + branch-gate.md, spec-template.md
