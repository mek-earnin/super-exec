...
Rules:
- Plan activation records `active_plan`, canonical sibling ledger path, and `ledger_status: pending`; se-plan creates and validates `deferred-findings-<plan-name>.md`, then marks status `ready`. For **plan-backed** execution, a pending, absent, or non-ready ledger blocks execution and returns to planning repair. **Skip-plan** execution has no plan ledger and must not be sent to planning repair for that absence; the governing artifact is the approved spec or the task spec. See [`core workflow`](../core-workflow/spec-core-workflow.md) and [`exec-from-spec-or-interview`](../../adr/exec-from-spec-or-interview.md).
