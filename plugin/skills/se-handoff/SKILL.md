...
3. **Write the handoff file.**
   Path: `.super-exec/<feature>/<plan-dir>/handoff.md` — the same directory that contains the `plan.md` being executed. Never write to `.super-exec/` root or anywhere else.
...
5. **Do NOT clear `.super-exec/active`.**
   The session is resuming, not ending. `active` must remain set so the session-start hook and se-exec can detect the in-progress plan and load the handoff. Only `se-pr` clears `active`, on PR completion.
...
| "I'll clear `active` so the next session starts clean" | Never. `active` must stay set. Only `se-pr` clears it. The resume flow depends on `active` pointing to the in-progress plan. |
...
| "I'll summarize the handoff from memory without checking the plan" | Open `plan.md` and verify task names and order before writing. Stale or misremembered task names break the resume. |
...
|---|---|---|
| Controller / handoff author (judgment, state capture) | strong non-fast (see `references/model-tiers.md`) | Default / inherit (non-fast) |
