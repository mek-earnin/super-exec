...
Cross-harness resume mechanism. /se-exec invokes se-handoff when the controller judges — via a **model-judged heuristic, not a fixed token count** — that continuing risks losing coherence or truncating verification evidence. Output: a single `handoff-<plan-name>.md` capturing every piece of state a fresh session needs to resume without re-litigating anything.
...
3. **Write the handoff file.**
   Path: `<root>/[<app>/]<feature>/plans/<YYYY-MM-DD>-<plan-name>/handoff-<plan-name>.md` — the same directory that contains the plan file (`plan-<plan-name>.md`) being executed (`<root>` ∈ {`docs/specs/`, `.super-exec/specs/`}; scan/write under the plan's root). Never write to `.super-exec/` root or anywhere else. Use the `Write` tool. See [handoff-template.md](./handoff-template.md) for the required sections. The template is copy-pasteable markdown only: it starts directly with `# Handoff — <plan-dir name>`, has no outer explanatory heading/prose, and no fenced code-block wrapper. Fill the sections in order. Under one `## Completed Tasks` section, repeat `### <task>` entries once per done task, include the full next-task description from the plan file (`plan-<plan-name>.md`), and write `None.` for empty in-flight decisions or follow-ups.
