...
**On every commit, the caller must have review evidence for exactly the staged paths.** Evidence is a **model-judged, in-context signal** — **no durable marker** (nothing like `.super-exec/gate-open`), and **any later change to those paths invalidates it** (a fix answering review feedback is itself unreviewed until a fresh reviewer returns clean). It exists only when the caller can tie the staged paths to a review that already covered them:

- **Already cleared by an `/se-review` pass** — valid only if **no change** followed the clean pass. `/se-review` blocked on Critical/Important and looped fix → fresh reviewer until clean, so those paths need no re-review. Covers **`/se-exec`'s per-task build loop** and **`/se-pr-triage`'s fix flow** — Track A / B fixes clear `/se-verify` + `/se-review` before the commit (`plugin/skills/se-pr-triage/SKILL.md`, `plugin/skills/se-pr-triage/ci-triage.md`). `/se-exec`'s loop orders **commit → review**, so review isn't literally done at commit time; it's safe because the loop **wraps** a blocking `/se-review` around every commit and won't hand the branch to `/se-pr` until Critical/Important are resolved and a fresh reviewer is clean. se-commit does not re-review.
