...
- [ ] **1. Start branch context warm-up (non-blocking)** — Read `${CLAUDE_SKILL_DIR}/branch-gate.md` (see [branch-gate.md](./branch-gate.md)) and start only its **Phase A: read-only warm-up**. This runs `${CLAUDE_SKILL_DIR}/branch-context` in the background when the harness supports background shell work. Do NOT wait for Atlassian MCP or git branch sampling before asking the first interview question. Do NOT create or switch branches here.
...
- [ ] **7. Collision gate (CRITICAL)** — Before preparing the spec draft, compare the candidate slug against the repo basename AND the `package.json` `name` field. On a match, **HALT** and confirm the real feature with the user via `AskUserQuestion`. Do not silently name a spec after the project.
- [ ] **8. Ticket + branch confirmation** — Complete `${CLAUDE_SKILL_DIR}/branch-gate.md` **Phase B: ticket + branch confirmation** after the last WHAT question. If no ticket was provided or inferred, ask once: "Do you have a Jira ticket for this task?" If Atlassian MCP is available and a ticket is known, use it to fetch the ticket title/description and derive the branch name. Present the ticket value (`NO_TICKET` if none) and proposed branch name once; accept edits now. This is the only normal branch-name question.
- [ ] **9. Prepare the spec draft to the template** — Prepare `docs/specs/<feature>.md` (or `docs/specs/<app>/<feature>.md` in monorepos) following [spec-template.md](./spec-template.md), but keep it in memory or under `.super-exec/` until after approval and branch creation. For an update, draft only the sections affected by the delta. Do NOT write committed artifacts on the old branch.
- [ ] **10. Stage deferred glossary / ADR edits in the draft** — If the interview resolved new domain terms, include the intended `CONTEXT.md` edits in the draft bundle; create the real file only after approval and branch creation. Offer an ADR only when ALL THREE hold: (a) the decision is hard to reverse, AND (b) it would be surprising without context, AND (c) it resulted from a real trade-off between alternatives. ADRs live at `docs/adr/NNNN-<slug>.md`, but do not write them yet.
- [ ] **11. Spec self-review** — Before presenting the spec to the user, scan the draft for: placeholders (TBD / TODO / "to be determined"), internal contradictions, scope creep (any HOW that snuck in), **any code (fenced code blocks, file layout, scaffolding — the spec is WHAT-only)**, and ambiguity (any requirement readable two ways). Fix the draft. Move any code/HOW to a note for se-plan and remove it from the spec. A requirement readable two ways must be made explicit — pick one reading and state it.
- [ ] **12. Spec-approval gate (review/fix loop), then branch + commit + auto-chain to se-plan** — Present the finalized spec and ask via `AskUserQuestion`: **"Everything look good? Proceed to planning?"**
  - **Change requested** → fix the draft, re-present, ask again via `AskUserQuestion`. Loop until the user explicitly approves (e.g. "looks good" / "yes" / "approve").
  - **On approval** → complete `${CLAUDE_SKILL_DIR}/branch-gate.md` **Phase C: create/switch confirmed branch**, then write/update the approved artifacts using `Write` or `Edit`, then **commit the spec** via **`se-commit`**, passing the paths this approval touched (the spec + any ADR / `CONTEXT.md` file). **UNLESS** the user asked not to commit, or `docs/specs/` is gitignored (in either case, skip the commit and say so). The user's approval IS the authorization to create/switch to the already-confirmed branch and commit; never commit before approval or while changes are pending.
...
| "I already confirmed the branch, I'll ask again before committing" | Re-prompt for the same branch at approval time | Do not ask again unless branch creation fails or facts changed. The earlier confirmation plus spec approval authorizes creating/switching to that branch and committing. |
| "I'll write the spec file now and move branches later" | Persist `docs/specs/`, `CONTEXT.md`, or ADR edits before branch creation | Keep drafts in memory or `.super-exec/`; write committed artifacts only after approval and branch creation. |
