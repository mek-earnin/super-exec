...
- [ ] **9. Write / update the spec file to the template** — Write `docs/specs/NNNN-<feature>.md` (or `docs/specs/<app>/NNNN-<feature>.md` in monorepos) using the `Write` or `Edit` tool, following [spec-template.md](./spec-template.md). `NNNN` is the next available number in that directory (find the highest existing four-digit prefix and increment). For an update, keep the existing number and edit only the sections affected by the delta. This uncommitted file is the review artifact; do NOT commit it before approval.
