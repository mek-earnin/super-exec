...
- [ ] **9. Write / update the spec file to the template** — Write `docs/specs/<feature>.md` (or `docs/specs/<app>/<feature>.md` in monorepos) using the `Write` or `Edit` tool, following [spec-template.md](./spec-template.md). For an update, edit only the sections affected by the delta. This uncommitted file is the review artifact; do NOT commit it before approval.
