...
Invoking this skill — whether user typed `/se-discuss` or model auto-invoked it to start discussing a feature — **means a super-exec design session is starting**. As very first action, **write the session marker** `.super-exec/active` in repo root (create `.super-exec/` if needed); small payload enough, e.g. `phase: discuss` and `started: <current UTC ISO-8601>`. Use `Write` tool. Existence + mtime are what hooks read; stale-marker decision is a model-judged heuristic with no fixed TTL. **Immediately after writing the marker, invoke `/se-local-ignore`.** These are the only side-effects at entry; behavior identical for manual and auto invocation.
...
Dispatch finder subagents (via `Task` tool) to research: existing domain terms in `CONTEXT.md` (or `CONTEXT-MAP.md` for multi-context repos), related specs in `docs/specs/`, relevant source files, naming conventions, any code touching the area of work. Use `Read`, `Bash`, `Grep` inside these subagents. Do NOT ask user for anything discoverable by reading the codebase.

## 3. New-vs-update detection

Glob `docs/specs/` (and `docs/specs/<app>/` in monorepos). Match existing spec by ticket key in header AND by feature slug after numeric prefix. Confirm with user via `AskUserQuestion`: "I found `docs/specs/0007-payment-retry.md` — is this an update to that spec, or a new one?" An **update** interviews only the **delta**; do not re-interview the whole spec.
...
For a multi-feature split, additionally check: each spec is cohesive (no unrelated capabilities crammed together, no tightly-coupled capability split across two specs), numbering reflects dependency order, and every `Depends on` reference points to a real sibling spec.
...
ADR file path: `docs/adr/NNNN-<slug>.md` where `NNNN` is the next available number (find the highest existing number and increment). Write the ADR content following [@./adr-format.md](./adr-format.md).
