...
- [ ] **1. Choose the PR creation path.** Inspect the repo's `.claude/skills/` directory (via a finder subagent) for a `create-pr` skill. Two paths:
...
- [ ] **3. PATH B — Build the PR draft (no repo `create-pr` skill).** See [pr-template.md](./pr-template.md) for the default title format and body rules. Summary:
  - **Title** — `<type>(<optional-scope>): <brief summary> - <TICKET-a>, <TICKET-b>`. Types: `feat`, `fix`, `ci`, `test`, `chore`. Scope: in a monorepo include the changed app/package scope only when **exactly one** app changed (omit when multiple changed); in a **standalone repo omit the scope entirely**. Tickets: comma-separated; `NO_TICKET` if none.
  - **Body** — locate `pull_request_template.md` (typically `.github/pull_request_template.md`). Mirror its primary `##` sections **1:1**: exactly that many primary sections, same headings, same order. No extra primary sections. Sub-sections within a section are allowed. If no template exists, write a concise structured description covering: what changed, why, how to test.
...
| "No repo skill, so I'll wing the PR title/format" | Write a free-form PR description or an ad-hoc title when no `create-pr` skill is present | Path B always follows `pull_request_template.md` strictly (mirror its primary sections 1:1) and the default title format per [pr-template.md](./pr-template.md) (scope omitted entirely in a standalone repo). No template at all → minimum structured description (what / why / how to test), draft mode, with evidence. |
