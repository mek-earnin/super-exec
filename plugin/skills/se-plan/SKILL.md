...
- [ ] **1. Read the spec** — When auto-chained from se-discuss, the spec just written is the one to plan. On a manual re-entry, locate the spec at `docs/specs/<feature>.md` (or `docs/specs/<app>/<feature>.md` in monorepos). Use the `Read` tool to read it in full. Identify all acceptance criteria. Do NOT proceed if no spec file is found on disk — tell the user to run `/se-discuss` first. (A spec that exists but is uncommitted is acceptable; only a genuinely missing spec blocks.)
