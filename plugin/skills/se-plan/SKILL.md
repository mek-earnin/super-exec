...
**Consult config for placement.** Before writing, invoke `/se-get-config` and read `commitPlan`. A plan's placement resolves against its governing spec's on-disk root: **a plan is never committed while its spec is local.** Use the physical root where step 1 already located the governing spec — read that located spec's root, do not re-derive it.

- **`"inheritSpec"`** → place the plan under the SAME physical root as the governing spec: committed `docs/specs/` if the spec is committed, local `.super-exec/specs/` if the spec is local. No prompt either way.
- **`false`** → place the plan under the local root `.super-exec/specs/`, regardless of the spec's root. No prompt.
- **`"ask"`** — depends on the spec's root:
  - Spec is **local** (`.super-exec/specs/`) → place the plan under the local root `.super-exec/specs/`. No prompt — a committed plan may never sit over a local spec.
  - Spec is **committed** (`docs/specs/`) → prompt via `AskUserQuestion` with two options:
    1. **(Recommended) Keep the plan local** under `.super-exec/specs/` — a plan is typically a one-off for this single implementation, so it needn't be committed.
    2. **Commit the plan** under `docs/specs/` alongside its spec — shares the plan for review, matching the committed spec.
    Use whichever root the user picks.

The resolved root ALSO drives whether step 11 commits the approved plan (commit only when the plan lives under `docs/specs/`; a local `.super-exec/specs/` plan is intentionally uncommitted-by-design).
