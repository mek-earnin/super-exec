---
name: se-verify
description: Use when /se-exec has just completed an implementer task and needs to confirm it is green.
---

# se-verify

se-verify is the **inner verification loop** /se-exec invokes after an implementer subagent finishes a task. Strict runner→judge cycle: cheap runner subagent runs verification in its own throwaway context → strong judge evaluates the returned evidence → if judge finds anything short of green, a fixer subagent addresses the specific failure, then the cycle repeats. Loop until judge passes with shown evidence → report **inner-loop-green**. If judge finds a failure is an implementation limitation (spec unachievable as written — not a fixable bug), report **limitation-blocked** to /se-exec instead of looping the fixer.

**Checkpoint definition:** *inner-loop-green = baseline AND task-specific verification both pass, with the runner's actual evidence shown and the judge's explicit pass rendered.*
...
Confirm you have: (a) the task description from the plan; (b) the plan's **verification design** — the recorded baseline commands (e.g. `lint:fix` + `type-check`, or `dotnet build`) and the task-specific verification method (test suite, browser check, endpoint call, or custom script). If either is missing, halt and ask /se-exec to supply them before proceeding.

## 2. Dispatch the runner subagent

See the `/se-subagent` skill for the model and dispatch details. Dispatch with the `Task` tool. Give the runner:

- The exact baseline commands from the plan's verification design — run first.
- The task-specific verification commands/steps from the plan's verification design — run second.
...
Wait for the runner to return. Accept only structured EVIDENCE — exit codes, counts, relevant log lines, file paths. If the runner returns an interpretation ("it passed") without hard evidence, reject it and re-dispatch with an explicit instruction to include exit codes and output excerpts.

## 4. Dispatch the judge subagent

See the `/se-subagent` skill for the model. Dispatch with the `Task` tool. Supply the judge:
...
See the `/se-subagent` skill. Dispatch with the `Task` tool. Give the fixer:
