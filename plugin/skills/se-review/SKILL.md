...
Outer review loop invoked by /se-exec after a task is committed and /se-verify has declared inner-loop-green. Cycle: a fresh reviewer subagent checks architecture conformance first → structured deep-review pass. Findings are severity-tagged; Critical/Important block. Fixes re-enter the inner loop (/se-verify's fix path); a fresh reviewer re-reviews from scratch after every fix cycle. Loop runs until a fresh reviewer declares the work clean.

**Reviewers always run as subagents.** The controller dispatches a fresh reviewer subagent with the `Task` tool, waits, and reads the returned summary. Heavy or long review never runs inline in the controller.

**Realism filter (always active):** a finding is valid only if the failure state it describes can actually be reached given the code as written and the inputs the system can receive. Never flag impossible or unreachable cases — a negative index into an internally-controlled array the caller cannot influence, an error path that requires an input the API contract forbids, a race condition that cannot occur in a single-threaded runtime. A finding that requires an impossible state is not a finding. State that dismissal explicitly in any report that dismisses such a case.

---

## Mandatory Ordered Checklist

Complete every item in order. Never skip or reorder. Do not advance past a step until it is confirmed done.

- [ ] **1. Arch-gate — dispatch reviewer**
...
The reviewer checks structural shape — component responsibilities, data-flow direction, module boundaries, interface signatures — against the plan's agreed design. It does not yet assess correctness, bugs, or style.

## 2. Arch-gate verdict — DEVIATES handling

If the arch-gate reviewer renders DEVIATES, the work does not proceed to deep review. Instead:

- Dispatch the inner loop fix path (/se-verify's fixer step) targeting only the specific structural deviation.
...
Once the arch-gate is clean, dispatch a fresh reviewer subagent — not the arch-gate reviewer — with the full implementation diff and the spec acceptance criteria. Instruct the reviewer to assess in this exact order:
...
**(b) Reuse-existing-utility check.** Did the implementation hand-roll something a repo utility, helper, or installed skill already provides? Dispatch a finder subagent (cheap / finder tier) with the `Task` tool to enumerate relevant repo utilities, helpers, and `.claude/skills/` entries before rendering this sub-finding. A hand-rolled duplicate of an existing repo utility is always at least an Important finding.

**(c) Pattern and consistency check.** Does the implementation follow codebase conventions — naming, error handling style, logging, module structure, test patterns? Inconsistency that would confuse a future maintainer is a Minor finding unless it creates a correctness risk.
...
Effects (full definitions in Severity Reference below):

- **Critical** — BLOCKS. Must be fixed before reporting clean.
- **Important** — BLOCKS. Must be fixed before reporting clean.
- **Minor** — NON-BLOCKING. Reported in the final summary; does not trigger a fix loop.

If there are no Critical or Important findings, skip to step 7.

## 5. impeccable UI-critique lens (UI work + installed)

Run this step if and only if the feature involves UI work (derive from the plan/spec and the diff). **Never ask the user whether to run it — run it automatically when impeccable is installed, skip it when it is not.**
...
- **If present:** dispatch a reviewer subagent with the `Task` tool that runs impeccable `critique` (or `audit`, per the installed skill's interface) against the UI changes. The reviewer returns findings tagged Critical / Important / Minor using the same severity rules. These findings enter the fix loop on the same terms as deep-review findings — Critical/Important block; Minor is reported only.

impeccable is the only allowed optional external tool at this step; no other external tools are invoked here.

## 6. Critical/Important findings — re-enter inner loop

For each Critical or Important finding (including any from the impeccable lens):
...
- Dispatch the inner loop fix path (/se-verify's fixer step) with the `Task` tool, providing the fixer: the specific finding (what is wrong, where, and what the evidence showed), the relevant acceptance criteria, and the plan's architectural context. Do NOT provide the full review; give only what is needed to fix the specific issue.
...
After any fix cycle (step 2 or step 6), the re-review is always performed by a NEW subagent dispatched with the `Task` tool that has not seen the prior review conversation. Provide the fresh reviewer with the full current diff and the spec. It runs the same deep-review sequence (steps 3a → 3b → 3c) plus the impeccable lens when the work is UI and impeccable is installed. Loop until a fresh reviewer finds zero Critical or Important findings.

## 8. Report clean

Only when a fresh reviewer has returned zero Critical or Important findings, perform a final spec-match assertion before closing the loop:
...
| "The architecture is close enough — it's basically what the plan said." | No "close enough" at the arch-gate. CONFORMS or DEVIATES. A deviation, however small in your judgment, re-enters the inner loop and re-gates. |
...
| "This negative-index / impossible-input case is a bug." | Apply the realism filter. An index into an internally-controlled array the caller cannot influence, or an error path gated by an API contract, is an unreachable case. Unreachable cases are not findings. Document the dismissal explicitly. |
