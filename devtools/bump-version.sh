...
#
# Usage:
#   devtools/bump-version.sh <new-version>   Bump all declared files to new version
#   devtools/bump-version.sh --check         Report current versions (detect drift)
#   devtools/bump-version.sh --audit         Check + grep repo for old version strings
#
set -euo pipefail
...
case "${1:-}" in
  ...
  --audit)
    ...
    ;;
  --help|-h|"")
    echo "Usage: devtools/bump-version.sh <new-version> | --check | --audit"
    echo ""
